...
// 用户列表
const receiveUserList = (user) => ({ type: RECEIVE_USER_LIST, data: user });
//异步获取用户列表
...
/* 
  初始化客户端socketio
  1. 连接服务器
  2. 绑定用于接收服务器返回的chatMsg的监听
*/
function initIO(dispatch, userid) {
  if (!io.socket) {
    io.socket = io("ws://localhost:4000");
    io.scoket.on("receiveMsg", (chatMsg) => {
      if (chatMsg.from === userid || chatMsg === userid) {
...
async function getMsgList(dispatch, userid) {
  ...
  const result = response.data;
  if (result.code === 0) {
    const { chatMSg, users } = result.data;
    dispatch(receiveMsgList({ chatMsg, user, userid }));
  }
}
